...
/**
 * 포장(bsc05) 서비스 — 블리스터(ProductionCreateService) 방식으로 재구성.
 *
 * ═══ 폐기된 테이블과 그 값의 행선지 ═══
 *   pack_session  ✗   Phase     → 파생(재고가 움직인 흔적이 곧 단계)
 *                     NoJobYN   → 헤더 작지 "SourceTableName"='pack'
 *                     Units     → mat_produce."GoodQty"
 *                     OutboxCap → getBoxSpec() 이 BOM 에서 매번 계산
 *   pack_pk       ✗   담아둔 PK → pack_alloc_item."ItemKind"='pk'
 *   pack_carton   ✗   카톤      → 저장 안 함. 완제품수량 ÷ 입수 로 완전 복원
 *
 * ═══ 공정 흐름 (v2 — 완제 시점이 CK생산으로 이동) ═══
 *   ① PK 담기      PK 로트 저장(재고 이동 없음). BOM 의 PK 품목이 자동 선택된다
 *   ② 인박스       PK + IN BOX 소비 · UDI 라벨 2회 스캔        ★첫 재고 이동
 *   ③ CK 생산/투입  CK 소비 → 완제품 로트(제품창고 4)          ★완제 시점
 *                   국가별로 「새로 생산」 또는 「자체재고 투입」 중 택일
 *   ④ 아웃박스      OUT BOX 소비 + 세션 완료. 새 로트 없음
 *
 * ═══ 단계 파생 (getPhase / PHASE_SQL) ═══
 *   mat_produce."State"='finished'          → done
 *   pack_alloc."CkState"='produced' 존재     → outbox  (CK까지 끝 = 완제품 나옴)
 *   pack_label(ckpk/inbox) 존재              → ck      (인박스 끝)
 *   pack_alloc_item(ItemKind='pk') 존재      → inbox   (PK 담김)
 *   그 외                                    → pk
 *
 ...
 *   ckstock CK 자체재고 세션 — 작지에 부모가 없다(Parent_id IS NULL).
 *           ③ CK 생산만 하고 생산창고(17)에 재고로 남긴다. 나중에 kit 세션이 투입한다
 *
 * ═══ PK 로트 저장 위치 ═══
 *   pack_alloc_item."ItemKind"='pk' + "MatProduce_id"(세션 직결).
 *   PK 는 국가로 갈리지 않는다. 예전엔 PackAlloc_id 가 NOT NULL 이라 첫 배분 행에
 *   억지로 매달았는데, ①에서 저장하려면 배분이 아직 없다 → 세션에 직접 맨다.
 */
@Service
public class PackService {
	 ...
	 *
	 * avail = CurrentStock − (다른 세션이 ②CK생산까지 마치고 아직 ③인박스를 안 한 예약분)
	 *
	 ...
	 *
	 *   ★ 필터백(ck 자재)은 ②에서 저장과 동시에 소비되므로 예약분이 없다.
	 *     PK 만 ②저장 ~ ③소비 사이에 떠 있어 예약 계산 대상이다.
	 */
	public List<Map<String, Object>> getSterilizedLots(String kind, Integer materialId,
		...
		return this.sqlRunner.getRows("""
            SELECT ml.id                                    AS mat_lot_id
              ...
              -- 다른 세션이 ①에서 담아두고 아직 ②인박스로 소비하지 않은 PK 수량
              LEFT JOIN LATERAL (
                  SELECT COALESCE(SUM(pai."Qty"),0) AS rq
                   ...
                   WHERE pai."MatLot_id" = ml.id
                     ...
                     AND NOT EXISTS (SELECT 1 FROM pack_label pl
	...
	// =========================================================================
	// 작업 목록 (화면 A) — 작지 + 무작지 대기 로트를 한 목록에
	// =========================================================================
	...
	public List<Map<String, Object>> getOrderList(String dateFrom, String dateTo,
		...
		return this.sqlRunner.getRows("""
            ...
            SELECT jr.id                        AS id
                 ...
                 -- 작지 없이 시작한 건인지 (pack_session.NoJobYN 대체)
	...
	/**
	 * 세션 상세 — 단계 + PK 로트 + 국가배분 + CK 투입 + 라벨 + 카톤(계산) (화면 C 복원)
	 *
	 *   ★ PK·배분은 ②CK생산 이후에만 존재한다. ①은 화면 메모리라 빈 배열이 정상.
	...
	public Map<String, Object> getSessionDetail(Integer mpId, String spjangcd) {
		...
		if (cap <= 0) cap = 4f;
		if (session != null) session.put("outbox_cap", cap);
		...
		// ① 자동선택용 — 이 세션이 담아야 할 PK 품목 (산출품 BOM 의 블리스터 산출물)
		...
		data.put("cartons", buildCartons(units, cap, session));
		return data;
	}
	 ...
	 */
	private List<Map<String, Object>> buildCartons(float units, float cap, Map<String, Object> session) {
	 ...
	 *
	 *   ★ PK 로트는 저장하지 않는다(화면 메모리). 여기서는 계획수량 계산에만 쓴다.
	 *     실제 저장은 ②CK생산에서 pack_alloc_item(ItemKind='pk') 로 한 번에 들어간다.
	 */
	public AjaxResult packStartNoJob(Integer productMatId, Float orderQty,
		...
		float pkSum = 0f;
		for (Map<String, Object> pl : pkLots) pkSum += toFloat(pl.get("qty"));
		...
		hdr.setSourceTableName("pack");      // ← 「작지 없음」 판별 키 (NoJobYN 대체)
	...
	private MaterialProduce createSessionMp(JobRes jr, Integer productMatId,
		...
		mp.set_status("a");
		mp.setStoreHouseId(resolveOutStore(jr.getWorkCenter_id()));
	...
	/**
	 * 이 세션이 담을 PK 로트를 저장한다.
	 *
	 *   재고는 움직이지 않는다. ②인박스에서 소비된다.
	 *   ★ 그래도 서버에 저장하는 이유: 새로고침·다른 태블릿 진입에도 남아야 하고,
	 *     다른 세션이 같은 로트를 담지 못하게 예약 효과가 필요하다.
	 *   ★ 국가 배분이 아직 없으므로 pack_alloc_item 을 세션(MatProduce_id)에 직접 맨다.
	 *
	...
	public AjaxResult savePkLots(Integer mpId, List<Map<String, Object>> pkLots,
		...
		if (!"pk".equals(phase) && !"inbox".equals(phase))
			return fail(result, "인박스 포장 이후에는 PK 를 바꿀 수 없습니다.");
		...
		data.put("phase", resolved.isEmpty() ? "pk" : "inbox");
		result.data = data;
		return result;
	}
	 ...
	 * 화면이 진입 시 이 품목의 멸균창고 로트를 전부 체크해 둔다.
	 */
	...
	// =========================================================================
	// ② 인박스 — PK + IN BOX 소비 ★첫 재고 이동. 산출은 없다(완제는 ③)
	// =========================================================================

	/**
	 * 인박스 포장 — 담아둔 PK 와 IN BOX 를 소비하고 UDI 라벨을 남긴다.
	 *
	 *   ★ 여기서는 완제품 로트를 만들지 않는다. 아직 CK 가 안 들어갔기 때문이다.
	 *     완제품 산출은 ③ CK 생산/투입에서 한다.
	 *   ★ 라벨 행(ckpk/inbox)의 존재가 곧 '인박스 완료' 표식이다(단계 파생).
	 *
	 * @param labels [{kind:'ckpk'|'inbox', gtin, lot, date, expiry, qty, raw}]
	 */
	public AjaxResult inboxFinish(Integer mpId, List<Map<String, Object>> labels,
		...
		if (pkRows.isEmpty()) return fail(result, "담긴 PK 로트가 없습니다.");

		float units = 0f;
		Set<String> pkUdiSet = new LinkedHashSet<>();
		...
		for (Map<String, Object> pr : pkRows) {
			...
			if (avail < qty)
				return fail(result, "PK 로트 재고 부족 — " + lot.get("lot_no")
															+ " (필요 " + fmt(qty) + " / 가용 " + fmt(avail) + ")");
			String udi = str(lot.get("udi_lot"));
			if (!isBlank(udi)) pkUdiSet.add(udi);
			Map<String, Object> row = new HashMap<>(lot);
			row.put("_qty", qty);
			pkResolved.add(row);
			units += qty;
		...
		// ── 라벨 검증 ──
		String udiLot   = pickLabel(labels, "lot");
		String scanGtin = pickLabel(labels, "gtin");
		if (REQUIRE_UDI_LABEL && udiLot == null)
			return fail(result, "완제품 UDI 라벨 스캔이 필요합니다.");

		String expectGtin = getExpectedGtin(mp.getMaterialId(), spjangcd);
		if (expectGtin != null && scanGtin != null && !expectGtin.equals(scanGtin))
			return fail(result, "GTIN 불일치 — 이 작업지시의 완제품 라벨이 아닙니다. (기대 "
														+ expectGtin + " / 스캔 " + scanGtin + ")");
		if (udiLot != null && !pkUdiSet.isEmpty() && !pkUdiSet.contains(udiLot))
		...
		Integer inboxMatId = inbox == null ? null : toInt(inbox.get("mat_id"));
		float inboxNeed = 0f;
		if (inboxMatId != null) {
		...
		for (Map<String, Object> lot : pkResolved) {
			...
			writeConsumeRecord(jr, mp, toInt(lot.get("mat_id")), qty, STORE_STERIL,
				str(lot.get("lot_no")), "인박스 PK 투입(멸균창고)", startTs, endTs, user, spjangcd);
		}
		...
		// ★ 라벨 저장이 곧 '인박스 완료' 표식 = 단계가 ck 로 넘어간다
		saveScanLabels(mp.getId(), labels, spjangcd, user);

		// 수량만 잡아둔다. 아직 산출(GoodQty)은 아니다 — 완제는 ③에서
	...
	/**
	 * 인박스 취소 — PK·IN BOX 소비를 되돌리고 ① 로 복귀.
	 *
	 *   담아둔 PK 행은 남긴다 — 다시 고를 필요 없이 라벨만 다시 스캔하면 된다.
	 */
	public AjaxResult inboxCancel(Integer mpId, User user) {
		AjaxResult result = new AjaxResult();
		result.success = true;
		...
		String phase = getPhase(mpId);
		if (!"ck".equals(phase))
			return fail(result, "인박스 완료 상태에서만 취소할 수 있습니다.");

		rollbackProduce(mpId);   // PK·IN BOX 소비 복원 (산출 로트는 아직 없다)
	...
	/**
	 * 국가별로 CK 를 마련해 소비하고 완제품 로트를 산출한다.
	 *
	 *   국가 1건당 두 갈래 (화면이 mode 로 지정):
	 *     produce — CK 반제품을 새로 생산(구성자재 소비) 후 그 로트를 소비
	 *     stock   — 이미 생산창고(17)에 있는 CK 자체재고 로트를 그대로 소비
	 *
	 *   ★ kit 세션이면 여기서 완제품 로트가 나온다(제품창고 4).
	 *     ckstock 세션이면 생산까지만 하고 재고로 남긴다(→ ckStockFinish).
	 *
	 * @param allocations [{country, country_id, country_name, qty, ck_lot, mode,
	 *                      ck_mat_lot_id,                 // mode='stock' 일 때
	 *                      items:[{mat_id, qty, mat_lot_id}]}]  // mode='produce' 일 때
	 */
	public AjaxResult ckProduce(Integer mpId, List<Map<String, Object>> allocations,
															User user, String spjangcd) {
		AjaxResult result = new AjaxResult();
		result.success = true;
		...
		if (allocations == null || allocations.isEmpty())
			return fail(result, "국가별 배분이 없습니다.");
		...
		String phase = getPhase(mpId);
		if (!ckStock && !"ck".equals(phase))
			return fail(result, "인박스 포장을 먼저 완료해주세요.");
		if (ckStock && !"pk".equals(phase) && !"inbox".equals(phase))
		...
		float units = ckStock ? sumAlloc(allocations) : pkTotal(mpId);
		if (units <= 0) return fail(result, "수량이 0 입니다.");

		float allocSum = sumAlloc(allocations);
		if (!ckStock && Math.abs(allocSum - units) > 0.0001f)
			return fail(result, "국가별 배분 합계(" + fmt(allocSum) + ")가 인박스 수량("
														+ fmt(units) + ")과 다릅니다.");

		Timestamp now = DateUtil.getNowTimeStamp();

		// ── 사전 검증: produce 모드 자재를 한 번에 확인한다 ──
		Map<Integer, Float> need = new LinkedHashMap<>();
		for (Map<String, Object> a : allocations) {
			if (!"produce".equals(str(a.get("mode")))) continue;
		...
		// 화면이 보낸 mode / 지정 CK 재고로트를 국가코드로 찾아 쓴다
		Map<String, Map<String, Object>> byCountry = new HashMap<>();
		for (Map<String, Object> a : allocations) byCountry.put(str(a.get("country")), a);
		...
		for (Map<String, Object> ar : allocRows) {
			...
			if ("stock".equals(mode)) {
				...
				if (ckLotId == null)
					return fail(result, country + " — 투입할 CK 재고 로트를 선택해주세요.");
				Map<String, Object> lot = getUsableLot(ckLotId, STORE_CK_OUT, mpId);
				if (lot == null || toFloat(lot.get("avail")) < ckQty)
					return fail(result, country + " — CK 재고가 부족합니다. (필요 " + fmt(ckQty) + ")");
				ckLotNo = str(lot.get("lot_no"));
				ckMatId = toInt(lot.get("mat_id"));
			} else {
				...
				ckMp.setStartTime(mp.getStartTime() != null ? mp.getStartTime() : now);
				ckMp.setEndTime(now);
				ckMp.setDescription("CK 생산(" + country + ")");
				ckMp.set_audit(user);
				...
				for (Map<String, Object> it : items) {
					...
					if (fixedLotId != null) {
						...
						if (lot == null)
							return fail(result, country + " · " + matName + " — 지정한 멸균 로트를 사용할 수 없습니다.");
						float avail = toFloat(lot.get("avail"));
						if (avail < qty)
							return fail(result, country + " · " + matName + " — 멸균 로트 재고 부족 (필요 "
			...
			if (!ckStock) {
				...
				writeConsumeRecord(jr, mp, ckMatId, ckQty, STORE_CK_OUT, ckLotNo,
					("stock".equals(mode) ? "CK 자체재고 투입(" : "CK 투입(") + country + ")",
					mp.getStartTime(), now, user, spjangcd);
			}

			MapSqlParameterSource up = new MapSqlParameterSource();
			up.addValue("allocId", allocId);
			up.addValue("ckMpId", ckMpId);
			up.addValue("ckMatId", ckMatId);
		...
		String udiLot = getLabelValue(mpId, "inbox", "LotNo");
		if (udiLot == null) udiLot = getLabelValue(mpId, "ckpk", "LotNo");
		Integer outStoreId = (mp.getStoreHouseId() != null) ? mp.getStoreHouseId() : STORE_PRODUCT;
		receiveLot(mp.getMaterialId(), mp.getLotNumber(), udiLot, units, outStoreId,
		...
		if (cartonCnt > 0 && isBlank(cartonLot)) {
			...
			saveLabel(mp.getId(), "carton",
				...
				getLabelValue(mpId, "inbox", "MakeDate"), null,
				cartonCnt, null, spjangcd, user);
		}
	...
	/**
	 * CK 생산/투입 취소 — 완제품 로트와 CK 소비를 되돌리고 ②(인박스 완료) 상태로 복귀.
	 *
	 *   생산한 CK 는 지운다. 자체재고에서 투입한 CK 는 재고로 되돌아간다.
	 */
	public AjaxResult ckCancel(Integer mpId, User user) {
		AjaxResult result = new AjaxResult();
		result.success = true;
		...
		// 완제품 입고 + 이 세션의 소비를 통째로 되돌린다.
		// ★ 인박스(PK·IN BOX) 소비도 같이 풀리므로 아래에서 다시 넣는다.
		List<Map<String, Object>> keepCons = this.sqlRunner.getRows("""
            SELECT mlc."MaterialLot_id" AS lot_id, mlc."OutputQty" AS qty, mlc."CurrentStock" AS cs
             ...
             WHERE mlc."SourceTableName"='mat_produce' AND mlc."SourceDataPk" = :mpId
               AND ml."StoreHouse_id" <> :ckStore
            """, new MapSqlParameterSource().addValue("mpId", mpId).addValue("ckStore", STORE_CK_OUT));
		...
		for (Map<String, Object> r : this.sqlRunner.getRows("""
			...
			this.matProduceRepository.deleteById(ckMpId);
		}

		// 배분·CK 투입자재 제거 → 단계가 'ck'(인박스 완료) 로 돌아간다
		this.sqlRunner.execute("""
		...
		// 카톤 라벨 제거 (스캔 라벨은 남긴다 — 인박스는 그대로다)
		this.sqlRunner.execute("""
            DELETE FROM pack_label WHERE "MatProduce_id" = :mpId AND "LabelKind" = 'carton'
            """, p);

		// 인박스 소비(PK·IN BOX)를 다시 기록한다 — 실물은 이미 박스 안이다
		for (Map<String, Object> c : keepCons) {
			consumeFixedLot(toInt(c.get("lot_id")), toFloat(c.get("qty")), toFloat(c.get("cs")),
				mp, user, mp.getSpjangcd());
		}
	...
	private boolean isCkStockSession(MaterialProduce mp, JobRes jr) {
		...
		MapSqlParameterSource p = new MapSqlParameterSource().addValue("matId", mp.getMaterialId());
		Map<String, Object> r = this.sqlRunner.getRow("""
	...
	/** 라벨 1행에서 값 하나 */
	private String getLabelValue(Integer mpId, String kind, String col) {
		MapSqlParameterSource p = new MapSqlParameterSource();
		p.addValue("mpId", mpId);
		p.addValue("kind", kind);
		Map<String, Object> r = this.sqlRunner.getRow(
			"SELECT \"" + col + "\" AS v FROM pack_label WHERE \"MatProduce_id\" = :mpId AND \"LabelKind\" = :kind", p);
		return r == null ? null : str(r.get("v"));
	}
	 ...
	 *     라벨은 화면이 인쇄용으로만 만들면 되고 DB 에 남길 필요가 없다.
	 */
	public AjaxResult outboxFinish(Integer mpId, String endTimeStr, User user, String spjangcd) {
		...
		if (!"outbox".equals(getPhase(mpId)))
			return fail(result, "③ CK 생산/투입을 먼저 완료해주세요.");
	 ...
	 *     예전 흐름(완제=인박스)에서는 rollbackProduce 로 통째로 지웠는데,
	 *     지금 그렇게 하면 ④를 취소했을 뿐인데 완제품이 사라진다.
	 *     되돌릴 대상은 OUT BOX 소비 하나뿐이다.
	 *   ★ 카톤 라벨도 남긴다 — 실물 박스에 이미 붙어 있고, 번호가 바뀌면 어긋난다.
	 */
	...
	/**
	 * 세션 삭제 — working 상태만.
	 * 작지 없이 시작한 세션은 자동발행한 작지 2행도 함께 정리한다(다른 세션이 없을 때).
	 */
	public AjaxResult packDelete(Integer mpId, User user) {
		...
		// ③ CK/완제품이 나와 있으면 먼저 되돌린다 (인박스 소비는 ckCancel 이 다시 넣어둔다)
		if (!"pk".equals(getPhase(mpId)) && !"inbox".equals(getPhase(mpId))) {
			AjaxResult ck = ckCancel(mpId, user);
			if (!ck.success) return ck;
		}

		// ② 인박스 소비(PK·IN BOX)까지 되돌린다
		rollbackProduce(mpId);

		// 담아둔 PK·라벨 정리
		this.sqlRunner.execute("""
            DELETE FROM pack_alloc_item WHERE "MatProduce_id" = :mpId
            """, p);
		...
		// ★ 자동발행 작지면 통째로 정리한다. 아니면 실적을 다시 집계해 완료를 풀어준다.
		//   안 하면 세션이 0건인데 작지는 「생산완료 100%」로 남아 카드가 계속 뜬다.
		if (!cleanupAutoJobRes(jrId, mpId)) {
	 ...
	 *
	 * ★ deletedMpId 를 세션 수에서 뺀다.
	 *   바로 앞의 JPA deleteById 가 아직 flush 되지 않아 raw SQL 에는 그 행이 보인다.
	 *   그대로 세면 mp_cnt>0 으로 판단해 정리를 건너뛰고, 작지가 영영 남는다.
	 *
	 * @return 작지를 실제로 지웠으면 true
	 */
	private boolean cleanupAutoJobRes(Integer jrId, Integer deletedMpId) {
		if (jrId == null) return false;
		MapSqlParameterSource p = new MapSqlParameterSource();
		p.addValue("jrId", jrId);
		p.addValue("delMp", deletedMpId);
		Map<String, Object> r = this.sqlRunner.getRow("""
            SELECT jr."Parent_id" AS hdr_id
                 , COALESCE(hdr."SourceTableName",'') AS src
                 , (SELECT COUNT(*) FROM mat_produce
                     WHERE "JobResponse_id" = jr.id
                       AND (CAST(:delMp AS integer) IS NULL OR id <> CAST(:delMp AS integer))) AS mp_cnt
              FROM job_res jr
              LEFT JOIN job_res hdr ON hdr.id = jr."Parent_id"
             WHERE jr.id = :jrId
            """, p);
	...
	private Integer firstAllocId(Integer mpId) {
		Map<String, Object> r = this.sqlRunner.getRow("""
	...
	private void writeAllocations(Integer mpId, List<Map<String, Object>> allocations,
		...
		MapSqlParameterSource dp = new MapSqlParameterSource().addValue("mpId", mpId);
		// 생산되지 않은(plan) 배분만 교체한다 — PK 행도 같이 지워진다(FK)
		this.sqlRunner.execute("""
            DELETE FROM pack_alloc_item WHERE "PackAlloc_id" IN
                   (SELECT id FROM pack_alloc
		...
		for (Map<String, Object> a : allocations) {
			...
			p.addValue("code", str(a.get("country")));
	 ...
	 *
	 *   예약 = ②CK생산까지 마치고 ③인박스를 아직 안 한 세션의 PK 행.
	 ...
	 *   예전엔 DELETE 후 INSERT 했는데, 그러면 kind 를 나눠 저장할 수 없다 —
	 *   carton 을 넣는 순간 ckpk/inbox 가 날아가고 단계 파생이
	 *   outbox → pk 로 되돌아가 세션이 처음으로 리셋된다.
	 *   있지도 않은 중복을 막느라 진짜 사고를 열어두는 셈이었다.
	 * ★ 라벨 행의 존재가 곧 '인박스 완료' 표식이다(단계 파생). 함부로 지우지 말 것.
	 *   (packCancel 만 의도적으로 지운다 — 거기선 단계를 되돌리는 게 목적이다)
	 */
	...
	/** 라벨 목록에서 값 하나 — 인박스 우선, 없으면 CK·PK */
	private String pickLabel(List<Map<String, Object>> labels, String key) {
		if (labels == null) return null;
		String fallback = null;
		for (Map<String, Object> l : labels) {
			String v = str(l.get(key));
			if (isBlank(v)) continue;
			if ("inbox".equals(str(l.get("kind")))) return v;
			fallback = v;
		}
		return fallback;
	}
	...
	/** 작지 양품 합계 재계산 + 지시량 충족 시 완료. 헤더 작지도 같이 맞춘다. */
	private void recalcJobRes(JobRes jr, User user) {
		...
		boolean met = allDone && orderQty > 0 && (good + defect) >= orderQty;

		if (met) {
			jr.setState("finished");
			jr.setEndTime(DateUtil.getNowTimeStamp());
		} else if ("finished".equals(jr.getState())) {
			// ★ 세션을 취소·삭제해 실적이 줄면 완료를 되돌린다.
			//   안 하면 「작업 0건인데 생산완료 100%」인 카드가 목록에 남는다.
			jr.setState(good > 0 ? "working" : "ordered");
			jr.setEndTime(null);
			if (good <= 0) jr.setStartTime(null);
		}
		jr.set_audit(user);
		this.jobResRepository.save(jr);

		// 헤더 작지도 맞춘다 — 안 하면 작지 화면 진행률이 영원히 0
		if (jr.getParentId() != null) {
			JobRes hdr = this.jobResRepository.getJobResById(jr.getParentId());
			if (hdr != null) {
				hdr.setGoodQty(good);
				hdr.setDefectQty(defect);
				if ("finished".equals(jr.getState())) {
					hdr.setState("finished");
					hdr.setEndTime(DateUtil.getNowTimeStamp());
				} else if ("finished".equals(hdr.getState())) {
					hdr.setState(good > 0 ? "working" : "ordered");
					hdr.setEndTime(null);
				}
				hdr.set_audit(user);
